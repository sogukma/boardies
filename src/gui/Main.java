...
public class Main implements Runnable {
	...
	private void doPurchase(Player p, MessageHandler MH) {
		while (hasPurchaseCard(p) && p.getAmountOfPurchases() > 0) {
			...
			boolean booleanZahlungsmittel = MH.send(p.getName() + " W�hle eine Kaufkarte, mit der du kaufen willst!");
		...
		//	int auswahlZumKaufen = c.scan((p.getName() + " Kaufe eine Karte!")); 
			...
			if (isMoneyCard(p, auswahlZahlungsmittel)) {
				...
				if (stock.getStock().get(auswahlZumKaufen).getWorth() <= p.getHand().get(auswahlZahlungsmittel)
					...
					p.addHand(copy);
					//hier wird von Dominion punktzahl erh�ht
	...
	private void doAction(Player p, MessageHandler MH) {
		...
		while (hasActionCard(p) && p.getAmountOfActions() > 0) {
			...
			MH.send("Sie haben f�r diese Runde noch " + p.getAmountOfActions() + " Aktionen zur Verf�gung.");

		//	int auswahl = ScannerInterface.scan(p.getName() + " Bitte w�hle eine Aktionskarte aus!");

		
			boolean booleanAuswahl = MH.send(p.getName() + " Bitte w�hle eine Aktionskarte aus!");
